...
export default function SearchListBoard({value, index, setsearchData, props, category}) {
    // console.log(value)
    const { schoolSearch,
         setSchoolSearch,
         sectorSearch, 
  ...
  const [api, setApi] = useState([]);
    ...
    return (<ListItem onClick={()=>{}}>
      <span>나의 일자리는 어디에?</span>
    {locationSearch2.map((item, index)=>(<SelectItems key={index }
    onClick={()=>{locationSearch2.splice(index,1); locationSearch.splice(index,1)}}
